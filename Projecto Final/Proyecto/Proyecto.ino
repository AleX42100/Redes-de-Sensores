...
// FTP Config:
char ftp_server[] = "192.168.10.104";   //check ip adress via the Mac Terminal : 'ifconfig -a'
char ftp_user[]   = "ESP32";
char ftp_pass[]   = "1TRe100";
ESP32_FTPClient ftp (ftp_server, ftp_user, ftp_pass, 5000, 2);

// WiFi network name and password:
const char* networkName = "Tenda_DF9160_5G_EXT2.4G";  //change to RSense in the lab
const char* networkPswd = "soulteam204";    //change to "" in the lab
...
void loop() {
  ...
  for (int i = 0; i < NUMBER_ACQ; i++) // each file : 20 samples, taken each 100ms, for a 2s sampling time
  {
    ...
    //  doc["Sensor"] = "Mag";
    doc["Value"]   = magAbs; //value we want to send (may varie)

    serializeJson(doc, stringJson);
    stringJson = stringJson.substring(0, stringJson.length() - 1); //delete the }
    stringJson.concat('\n');
  ...
  totalAvg = totalAvg / NUMBER_ACQ;
  oledScreenResults(totalAvg);
